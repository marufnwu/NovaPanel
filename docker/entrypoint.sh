#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════════╗
# ║  NovaPanel Docker Entrypoint — Verification-First Approach         ║
# ║                                                                     ║
# ║  This script prepares the container environment before handing      ║
# ║  control to supervisord. Every step is verified before proceeding.  ║
# ║                                                                     ║
# ║  Responsibilities:                                                  ║
# ║    1. Pre-flight checks (network, directories, secrets)             ║
# ║    2. Database initialization (MariaDB, PostgreSQL)                 ║
# ║    3. Service configuration (nginx, php-fpm, postfix, etc.)         ║
# ║    4. Panel database migrations & seeding                           ║
# ║    5. Launch supervisord to manage all daemons                      ║
# ║                                                                     ║
...
# ─── Signal Handling ─────────────────────────────────────────────────────
cleanup() {
    log "Caught termination signal — shutting down gracefully..."
...
print_banner() {
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║          NovaPanel Docker Container              ║${NC}"
    echo -e "${CYAN}║          Verification-First Entrypoint           ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
# ─── Helper: Generate a secret if not already present ────────────────────
ensure_secret() {
    local filepath="$1"
    local label="$2"
    local length="${3:-32}"
...
phase_preflight() {
    ...
    # 1c. Generate secrets (idempotent — only if not already present)
    ...
    for ver in "${PHP_VERSIONS[@]}"; do
        if [ -x "/usr/sbin/php-fpm${ver}" ]; then
            ...
            warn "PHP ${ver} FPM not found — skipping"
        fi
    done
    ...
    if [ -f "/etc/supervisor/conf.d/novapanel.conf" ]; then
        if supervisord -c /etc/supervisor/conf.d/novapanel.conf --check 2>/dev/null; then
            ...
            warn "Supervisord config check failed — proceeding anyway"
        fi
    fi
}
...
phase_database_init() {
    ...
    if wait_for_cmd "mariadb -u root -e 'SELECT 1'" "$DB_WAIT_TIMEOUT" "MariaDB"; then
        ...
        if ! mariadb -u root -e "USE mysql;" 2>/dev/null; then
            log "  Root has no password — setting it now"
        fi
...
SQL
        ...
        warn "MariaDB did not start — will retry via supervisord"
    fi

    # Stop temporary MariaDB — supervisord will manage it
    ...
    if [ ! -d "${PG_DATA_DIR}/base" ]; then
        ...
        if [ -d "${PG_DATA_DIR}/base" ]; then
            ...
            warn "PostgreSQL init may have failed — supervisord will attempt start"
        fi
    else
    ...
    if wait_for_port 127.0.0.1 5432 "$DB_WAIT_TIMEOUT" "PostgreSQL"; then
        ...
        warn "PostgreSQL did not start — will retry via supervisord"
    fi
...
NGINXEOF
    ...
    if [ -f "/etc/bind/named.conf" ]; then
        if named-checkconf /etc/bind/named.conf 2>/dev/null; then
            ...
            warn "BIND9 configuration has issues — named-checkconf output:"
...
F2BEOF
    ...
    if [ -f "/etc/redis/redis.conf" ]; then
        ...
        # Disable daemonize — supervisord manages the process
        sed -i 's/^#\?daemonize .*/daemonize no/' /etc/redis/redis.conf
        # Disable systemd supervision — supervisord handles it
...
APACHEEOF
        ...
        log "Apache not installed — skipping (Nginx-only mode)"
    fi
}
...
# ═════════════════════════════════════════════════════════════════════════
phase_panel_init() {
    log "━━━ Phase 4: Panel Database Init ━━━"
    ...
    if [ ! -f "${DATA_DIR}/novapanel.db" ]; then
        log "First run detected — running migrations and seeding..."
        ...
        node dist/db/migrate.js 2>&1 && ok "Migrations complete" || {
            warn "Migration failed — will retry on API start"
        }
        ...
        node dist/db/seed.js 2>&1 && ok "Seeding complete" || {
            warn "Seeding failed — will retry on API start"
        }
    else
        ok "Database already exists — skipping migrations"
    fi
